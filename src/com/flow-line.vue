...
<script>
  export default {
    ...
    methods: {
      draw() {
        const xList = this.pointList.map(item => item.x)
        const yList = this.pointList.map(item => item.y)
        ...
        this.pointList.forEach(point => {
          point.x = Math.floor(point.x - this.left)
          point.y = Math.floor(point.y - this.top)
        })
      ...
      drawArrow(size = 6, fillStyle = '#666666') {
        ...
        let ang = Math.atan((end.x - start.x) / (end.y - start.y))
        ctx.beginPath()
        if (end.y - start.y >= 0) {
          ctx.rotate(-ang)
        } else {

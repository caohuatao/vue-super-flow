...
// CONCATENATED MODULE: ./packages/index.vue?vue&type=template&id=7c30164c&
...
var Graph_Graph = /*#__PURE__*/function (_GraphEvent) {
  ...
  Graph_createClass(Graph, [{
    ...
    value: function getMatrix() {
      ...
      var nest = function nest(vertex) {
        return _this4.linkList.filter(function (link) {
          return link.start === vertex;
      ...
      this.nodeList.forEach(function (node) {
        ...
        var list = matrix[colIndex] ? matrix[colIndex] : matrix[colIndex] = [];
        list.push(node);
      });
      matrix.forEach(function (subList) {
        subList.sort(function (prev, next) {
          return prev.rowIndex - next.rowIndex;
        });
        subList.forEach(function (node, idx) {
          return node.rowIndex = idx;
        });
      });
      return matrix;
    }
    ...
    value: function vertical() {
      ...
      if (this.nodeList.length && this.vertex) {
        var vertexX = this.vertex.center[0];
        ...
        this.getMatrix().reduce(function (y, col) {
          var maxHeight = Math.max.apply(Math, Graph_toConsumableArray(col.map(function (node) {
            return node.height;
          })));
          var xList = [];
          var width = vertexX;
          xList.push(width);
          col.reduce(function (prevNode, currentNode) {
            width += Math.ceil(prevNode.width / 2 + DISTANCE.x + currentNode.width / 2);
            xList.push(width);
            return currentNode;
          });
          col.forEach(function (node, idx) {
    ...
    value: function horizontal() {
      ...
      if (this.nodeList.length && this.vertex) {
        var vertexY = this.vertex.center[1];
        this.linkList.forEach(function (link) {
          var start = link.start,
              end = link.end;
        ...
        this.getMatrix(true).reduce(function (x, col) {
          var maxWidth = Math.max.apply(Math, Graph_toConsumableArray(col.map(function (node) {
            return node.width;
          })));
          var yList = [];
          var height = vertexY;
...
var component = normalizeComponent(
  ...
  menuvue_type_template_id_1b6d653b_render,
  menuvue_type_template_id_1b6d653b_staticRenderFns,
  false,
  null,
  null,
  null
...
var node_component = normalizeComponent(
  ...
  nodevue_type_template_id_41c99322_render,
  nodevue_type_template_id_41c99322_staticRenderFns,
  false,
  null,
  null,
  null
...
/* harmony default export */ var lib_vue_loader_options_packagesvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    nodeIntercept: function nodeIntercept(node) {
      ...
      return function () {
        return _this.outputIntercept(node.getInterface(), _this.graph.getInterface());
      };
    },
